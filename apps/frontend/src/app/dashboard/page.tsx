      ...
      <div className="bg-gray-50 -m-4 lg:-m-6 p-4 lg:p-6">
        ...
        {role === 'SysAdmin' ? (
          ...
          <div className="space-y-6">
            ...
            {studyLesson && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-2" role="dialog" aria-modal="true" aria-label={studyLesson.title}><div className="flex h-[min(94vh,900px)] w-[min(98vw,1500px)] flex-col overflow-hidden rounded-xl bg-white shadow-2xl"><div className="flex shrink-0 items-center justify-between border-b border-gray-200 px-3 py-2"><h2 className="font-semibold text-gray-900">{studyLesson.title}</h2><button type="button" onClick={() => setStudyLesson(null)} className="rounded-lg p-2 text-xl text-gray-500 hover:bg-gray-100" aria-label="Tutup materi">×</button></div><div className="min-h-0 flex-1 bg-gray-100 p-1">{studyLesson.type === 'youtube' ? <iframe title={studyLesson.title} src={studyLesson.url} className="h-full min-h-[300px] w-full rounded-lg" allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen /> : <iframe title={studyLesson.title} src={studyLesson.url} className="h-full min-h-[420px] w-full rounded-lg" />}</div></div></div>}
          </div>
        ) : (
          <>
